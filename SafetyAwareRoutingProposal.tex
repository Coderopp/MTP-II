...
\caption{Log-scaled algorithmic scalability demonstrating the rapid computational intractability of exact MILP solvers against the polynomial scaling of metaheuristics and $O(1)$ constant-time inference of Deep Reinforcement Learning agents.}
\label{fig:scalability}
\end{figure}

\subsection{Algorithmic Scalability and Intractability}
Figure \ref{fig:scalability} illustrates the execution latency across varying order complexities ($N$). Expectedly, the exact Mixed-Integer Linear Programming (MILP) solver scales exponentially, hitting catastrophic time limits and memory exhaustion immediately past $N=20$. This empirically validates the necessity of heuristic or neural boundaries for q-commerce routing, where hundreds of orders burst within minute-long windows. While Metaheuristics (GA, ALNS) successfully preserve polynomial scalability, the simulated Deep Reinforcement Learning (DRL) agent proves superior for live operations, yielding an instant $O(1)$ routing derivation irrespective of topological density.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/2_pareto_frontier.png}
\caption{The Pareto Optimization frontier establishing the systematic mathematical tradeoff curve between absolute travel efficiency ($\lambda_1$) and vehicular collision risk exposure ($\lambda_2$) within the New Delhi road topology.}
\label{fig:pareto}
\end{figure}

\subsection{The Safety-Efficiency Pareto Tradeoff}
By methodically iterating the scalar weights within the multi-objective framework, we generated a highly continuous Pareto frontier (Figure \ref{fig:pareto}). The curve reveals a critical behavioral insight: minimal marginal sacrifices in pure delivery speed yield drastic, non-linear reductions in aggregate collision risk. Consequently, platforms insisting on absolute maximum velocity operate on the extreme precipice of the risk curve, actively generating systemic danger for negligible fractional time savings.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/4_density_risk_box.png}
\caption{Distribution of normalized operational risk exposure inherent to optimizing a standard capacity $N=30$ routing instance, categorized by the structural density and road hierarchy availability of distinct Indian cities.}
\label{fig:density}
\end{figure}

\subsection{Urban Topographical Vulnerabilities}
Deploying the identical safety-weighted algorithm across distinct cities (Figure \ref{fig:density}) exposes unique infrastructural vulnerabilities. Dense grid environments like Mumbai naturally possess heavily constricted minimum achievable risk scores compared to sprawling cities like Pune. The architecture thus mathematically establishes that flat algorithmic SLA enforcement (e.g., universal 10-minute delivery periods) across distinct urban topographies is algorithmically invalid and biologically hazardous to riders forced to operate under uniformly restricted deadlines within structurally volatile environments.
