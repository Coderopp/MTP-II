...
\begin{tikzpicture}[node distance=0.4cm and 0.5cm,
  solver/.style={draw=iitblue,fill=iitblue!10,rounded corners,
                 text width=2.6cm,align=center,font=\scriptsize,inner sep=4pt},
  contract/.style={draw=gacolor,fill=gacolor!8,rounded corners,
                   text width=7.0cm,align=center,font=\scriptsize,inner sep=6pt},
  inst/.style={draw=gray,fill=gray!10,rounded corners,
               text width=2.5cm,align=center,font=\scriptsize,inner sep=4pt},
  arr/.style={-Stealth,thick}]
  \node[inst] (inst) {\textbf{Instance}\\typed dataclass\\depots, customers\\super-arcs\\$Q,T_{\max},\bar{R},\bar{H}_k$};
  \node[solver,right=0.7cm of inst,yshift=1.3cm]  (milp) {\textbf{MILP}\\HiGHS/CBC\\PWL SOS2\\$N\leq50$};
  \node[solver,right=0.7cm of inst]               (ga)   {\textbf{GA}\\Giant-tour\\BF split\\200 gen};
  \node[solver,right=0.7cm of inst,yshift=-1.3cm] (alns) {\textbf{ALNS}\\4 destroy ops\\3 repair ops\\SA accept};
  \node[contract,right=0.7cm of ga] (eval)
    {\textbf{Shared Evaluation Contract}\\[3pt]
     \texttt{eval.objective.breakdown()}\\identical $F_1$ for all solvers\\[2pt]
     \texttt{eval.feasibility.validate()}\\strict constraint checker\\[3pt]
     \textit{Zero constraint violations}\\\textit{across all 428 result rows}};
...
\column{0.32\textwidth}
  {\scriptsize\textbf{MILP}: exact benchmark; PWL outer-envelope for $e^{\beta\tau}-1$ via SOS2}
\column{0.34\textwidth}
  {\scriptsize\textbf{GA}: OX1 crossover; swap/insert/reverse mutation; BF split enforces hard caps}
\column{0.32\textwidth}
  {\scriptsize\textbf{ALNS}: Shaw/worst/random/risk-cluster destroy; greedy \& regret-2/3 repair}
\end{columns}
\end{frame}
