...
\setbeamertemplate{footline}{%
  \leavevmode\hbox{%
    ...
    \begin{beamercolorbox}[wd=.34\paperwidth,ht=2.25ex,dp=1ex,center]{title in head/foot}%
      \usebeamerfont{title in head/foot}SA-VRPTW
    \end{beamercolorbox}%
...
\begin{enumerate}
  ...
  \item \textbf{BASM Calibration} --- reproducible public-source chain
    (MoRTH 2022 + Mohan et al.\ 2017 + OSM proxy) without iRAD microdata;
...
\column{0.47\textwidth}
  ...
  \begin{itemize}
    ...
    \item OSM proxy --- \textbf{no iRAD microdata needed}
  \end{itemize}
  \vspace{4pt}
...
\vspace{4pt}
\begin{itemize}
  \item \textbf{Bengaluru} (113 stores) --- most stable; lowest $F_1$; recommended for sensitivity studies
  \item \textbf{Gurugram \& Mumbai} --- elevated $F_1$ at $N\geq50$ due to fewer depot clusters
  \item \textcolor{gacolor}{\textbf{GA $\leq$ ALNS}} in $F_1$ across all cities at $N=200$ --- consistent
  \item \textbf{Pune} --- only $N=20$ in current run; insufficient to generalise
\end{itemize}
\end{frame}
...
\column{0.48\textwidth}
  ...
  \begin{itemize}
    \item BASM is a \textit{calibrated prior}, not a measurement --- no iRAD microdata
...
\column{0.48\textwidth}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Direct crash microdata} --- upgrade BASM if iRAD access obtained
  \end{enumerate}
\end{columns}
\end{frame}
